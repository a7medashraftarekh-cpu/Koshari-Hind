...
const menu = [
  ...
  {cat:"فراخ", name:"ربع فرخة", price:85, desc:"ربع فرخة مع الأرز أو الإضافات"},
...
function renderFilters(){
  document.getElementById("filters").innerHTML = cats.map(c=>`<button class="filter ${c===activeCat?'active':''}" onclick="setCat('${c}')">${c}</button>`).join("");
}
function setCat(c){activeCat=c; renderFilters(); renderMenu();}
function renderMenu(){
  const items = activeCat==="الكل" ? menu : menu.filter(x=>x.cat===activeCat);
  document.getElementById("menuGrid").innerHTML = items.map((x,i)=>`
    <article class="item">
      <div class="item-top"><h3>${x.name}</h3><span class="price">${x.price} ج</span></div>
      <p>${x.desc}</p>
      <button class="add" onclick="addToCart(${menu.indexOf(x)})">+ أضف للسلة</button>
    </article>`).join("");
}
...
function saveCart(){localStorage.setItem("kh_cart",JSON.stringify(cart)); renderCart();}
function renderCart(){
  ...
  document.getElementById("cartItems").innerHTML = cart.length ? cart.map(x=>`
    <div class="cart-row">
      <div><h4>${x.name}</h4><small>${x.price} ج × ${x.qty}</small></div>
      <div class="qty"><button onclick="changeQty('${x.name}',-1)">−</button><b>${x.qty}</b><button onclick="changeQty('${x.name}',1)">+</button></div>
    </div>`).join("") : '<div style="text-align:center;color:#999;padding:60px 0">السلة فاضية 😅<br>اختار أكلك من المنيو</div>';
}
...
function openWhatsApp(){
  if(!cart.length){alert("ضيف حاجة للسلة الأول 😄"); return;}
  const total = cart.reduce((s,x)=>s+x.price*x.qty,0);
  let msg = "السلام عليكم، عايز أطلب من كشري هند:%0A%0A";
  cart.forEach(x=>msg += `• ${x.name} × ${x.qty} = ${x.price*x.qty} جنيه%0A`);
